...
## Changelog
- 2026-06-09 — Found a second public leak: the ORIGINAL CAIO intel dashboard `dashboard.html` (root, ~100KB, DTC/Shopify + Cannabis CPG verticals, 7 tabs: Overview/Landscape/Competitors/Pivots/Passive Revenue/10 Ideas/Pitch Trainer) and `glossary.html` were served publicly at cowy.ai/<file> (200) — violating the "cannabis = compliance-constrained verticals publicly / never the C-word / no personal name on public pages" rule. Extended the Worker gate (_worker.js isGated) to cover /dashboard.html + /glossary.html; verified both now 401. This dashboard (designed for daily auto-updates, originally via an n8n workflow) is the one Patrick wants updated daily at 6am PST + kept current. OPEN: design the daily-refresh pipeline (write-path + ideally data-driven refactor); set the Cloudflare secret to unlock all gated pages.
